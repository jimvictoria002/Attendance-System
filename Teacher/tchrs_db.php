...
<body>
    ...
    <div class="main-container">
        <div class="main">
            <h1 class="ats">Attendance System</h1>
            <h2 class="ec">E-Classrooms</h2>
            <div class="classroom-container">
                
                    <?php 
                    
                    $query = "SELECT * FROM subjects WHERE subjects.teachers_id = {$_SESSION['teachers_id']}";
                    $result = $conn->query($query);
                    while($row = $result->fetch_assoc()){
                    ?>
                        <a href="tchrs_classroom.php?sub_id=<?php echo"{$row['sub_id']}"?>&section=atd_today" class="classroom">
                            <div >
                                <h1 class="course-code"><?php echo"{$row['sub_code']}";?></h1>
                                <p class="course-name"><?php echo"{$row['sub_name']}";?></p>
                                <p class="classroom-teacher"><?php echo"{$row['course']} {$row['year_level']} {$row['section']}";?></p>
            ...
            <div class="subject-sched">
                <table>
                    ...
                    <tbody>
                        ...
                        $query = "SELECT * FROM subjects_sched INNER JOIN subjects ON subjects_sched.sub_id = subjects.sub_id INNER JOIN teachers ON subjects.teachers_id = teachers.teachers_id WHERE teachers.teachers_id = {$_SESSION['teachers_id']} AND subjects_sched.weeks = '".date('l')."' ORDER BY STR_TO_DATE(subjects_sched.start_time, '%h:%i %p')";
                        $result = $conn->query($query);
                        ...
                        <tr>
                            ...
                            <td><?php echo"{$row['course']} {$row['year_level']}-{$row['section']}";?></td>
    ...
    <script>
        ...
        dropper.addEventListener('click', function(){
            if(todrop.style.display === "block"){
                ...
                user.style.color = "rgba(255, 255, 255, 0.852)";
            }else{
